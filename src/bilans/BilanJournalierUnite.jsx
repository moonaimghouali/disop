...
import {printBilanJournalierUnite} from './bilans'

const BilanJournalierUnite = ({data, setBilan}) => {
  ...
  const {id, nom, code} = useSelector(state=>state.system)
  const nom_region = "hasasjkl"
      ...
      <div  className='w-full flex-1  flex flex-col overflow-y-scroll '>
        ...
        <div className='flex flex-col pb-4 border-b border-gray-300'>
          <div className='flex flex-row w-full'>
            <div className='mt-3 text-lg font-semibold flex-1'>Bilan Production de l'Unite Journalier</div>
            {data.validation_xp && (<button onClick={(e)=> printBilanJournalierUnite(data, nom_region, nom, code, user)} className='px-2 bg-yellow-500 text-white font-semibold flex flex-row items-center rounded-sm gap-2'>
                <div>Exporter</div>
          ...
          <div className='mt-3'>Unite : <b>{nom}</b></div>
...
export default BilanJournalierUnite