...
// calculer la production Journaliere de l'Unite
export const calculProductionPuitsCorrigee = (production) => {
    let p = production
    

    p.map((unite)=>{
        let productionTotalePuitsUnite = 0
        unite.puits.map((puits)=>{
            productionTotalePuitsUnite += puits.production_unite_vm
        })

        unite.puits.map((puits)=>{
            // puits.production_unite_vm designe la production journaliere calculee par EP
            puits.taux_contribution = Number((puits.production_unite_vm / productionTotalePuitsUnite).toFixed(4))
            puits.production_corrigee = Number((unite.production_unite_vm * puits.taux_contribution).toFixed(3))

        })
        unite["production_puits"] = productionTotalePuitsUnite
    })

   return p
}

export const calculProductionPerimetres = (prodPuitCorrigee, journee_production) =>{
    let PerimetresProduction = new Map()

    prodPuitCorrigee.map((unite)=>{
        unite.puits.map((puits)=>{
